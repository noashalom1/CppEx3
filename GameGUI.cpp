...
void GameGUI::showTargetSelection(std::function<void(Player*)> action, bool includeCurrentPlayer) {
    ...
    for (Player* p : game.get_players()) {
        if (!p->is_eliminated() && (includeCurrentPlayer || p != current)) {
            ...
            btn.setAction([this, p]() {
                try {
                    targetAction(p);  // זה עלול לזרוק שגיאה
                    state = GUIState::InGame;
                    targetButtons.clear();
...
void GameGUI::run() {
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == Event::Closed)
                ...
                if (state == GUIState::Setup) {
                    if (addPlayerBtn->contains(x, y)) addPlayerBtn->execute();
                    if (startGameBtn->contains(x, y)) startGameBtn->execute();
                    ...
                    for (Button& btn : targetButtons) {
                        if (btn.contains(x, y)) {
                            try {
                                btn.execute(); // הפעלת הפעולה שהוגדרה ב-showTargetSelection
                                state = GUIState::InGame;
                                targetButtons.clear();
                            } catch (const GameException& e) {
                                inGameError = e.what();
                ...
                if (state == GUIState::Setup) nameBox->handleEvent(event);
            }
        }
        ...
        if (state == GUIState::Setup) {
            ...
            if (tempNames.size() >= 2) startGameBtn->draw(window);
...
void GameGUI::setupButtons() {
    ...
    Button undoTaxBtn("Undo Tax (By Goveror)", font, sf::Vector2f(180, 35), sf::Vector2f(800, 50));
    ...
    Button peekBtn("Peek & Arrest (By Spy)", font, sf::Vector2f(180, 35), sf::Vector2f(800, 100));
    peekBtn.setAction([this]() {
        Player* p = game.get_current_player();
        if (p->role() == "Spy") {
            for (Player* other : game.get_players()) {
                if (other != p && !other->is_eliminated()) {
                    static_cast<Spy*>(p)->spy_peek(*other);
                    break;
                }
            }
        }
    ...
    Button undoCoupBtn("Undo Coup (By General)", font, sf::Vector2f(180, 35), sf::Vector2f(800, 150));
    ...
    Button undoBribeBtn("Undo Bribe (By Judge)", font, sf::Vector2f(180, 35), sf::Vector2f(800, 200));
    undoBribeBtn.setAction([this]() {
        showTargetSelection([this](Player* target) {
            Player* p = game.get_current_player();
            if (p->role() == "Judge" && target->get_last_action() == "bribe") {
                static_cast<Judge*>(p)->undo_bribe(*target);
                actionMessage = p->get_name() + " undid bribe of " + target->get_name();
                game.next_turn();
                inGameError.clear();
            }
        });
