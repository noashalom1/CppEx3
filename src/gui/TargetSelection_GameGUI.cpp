 ...
 */
// void GameGUI::showTargetSelection(std::function<void(std::shared_ptr<Player>&)> action, bool includeCurrentPlayer, std::vector<std::shared_ptr<Player>> &targets)
// {
...
//     std::vector<std::shared_ptr<Player>&> finalTargets;

//     if (!targets.empty())
//     {
//         // Filter the provided targets based on includeCurrentPlayer
//         for (std::shared_ptr<Player>& p : targets)
//         {
...
//     for (std::shared_ptr<Player>& p : finalTargets)
//     {
...
void GameGUI::showTargetSelection(std::function<void(std::shared_ptr<Player>&)> action, std::vector<std::shared_ptr<Player>> &targets, bool includeCurrentPlayer)
{
    state = GUIState::TargetSelection;
    targetButtons.clear();
    targetAction = action;
    ...
    for(int i : finalTargetsIdx) {
        std::shared_ptr<Player>& p = targets[i];
        Button btn(p->get_name(), font, {200, 40}, {400, y});
        btn.setAction([this, &p]()
                      {
            try {
