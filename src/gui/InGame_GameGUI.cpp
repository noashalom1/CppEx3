...
//     for (std::shared_ptr<Player>& p : game.get_players())
//     {
//         if (p->role() == role)
//         {
...
//     float y = startY;
//     int count = 0;
//     for (std::shared_ptr<Player>& p : rolePlayers)
//     {
...
//         btn.setAction([this, std::ref(p), actionPerPlayer]()
//                       {
//             try {
//                 if (p->is_eliminated()) {
                                  ...
                                  std::function<void(std::shared_ptr<Player>&)> actionPerPlayer) 
{
    ...
    int i = 0;
    for (std::shared_ptr<Player>& p : all_players)
    {
        if (p->role() == role)
        {
    ...
    {
        std::shared_ptr<Player>& p = all_players[idx];
...
//                         { showTargetSelection([this](std::shared_ptr<Player>& target)
//                                               {
//         try {
//             std::shared_ptr<Player>& p = game.get_current_player();
//             p->arrest(target);  
...
//             inGameError.clear(); }, game.get_players()); });
...
//     showTargetSelection(
//         [this](std::shared_ptr<Player>& target)
//         {
//             std::shared_ptr<Player>& p = game.get_current_player();
//             try {
//                 p->coup(target);
...
//     // Governor: Undo Tax
//     y += 40 * addRoleActionButtons("Governor", "Undo Tax", y, [this](std::shared_ptr<Player>& p)
//                                    {
//     try {
...
//     // Judge: Undo Bribe
//     y += 40 * addRoleActionButtons("Judge", "Undo Bribe", y, [this](std::shared_ptr<Player>& p)
//                                    {
//     try {
...
//     y += 40 * addRoleActionButtons("Spy", "Peek and Disable", y, [this](std::shared_ptr<Player>& p)
//                                    {
//     // Filtering targets: only living players and not the player themselves
//     std::vector<std::shared_ptr<Player>> filteredTargets;
//     for (std::shared_ptr<Player>& target : game.get_players()) {
...
//     showTargetSelection(
//         [this, &p](std::shared_ptr<Player>& target)
//         {
//             try
//             {
...
//     // General: Undo Coup
//     y += 40 * addRoleActionButtons("General", "Undo Coup", y, [this](std::shared_ptr<Player>& p)
//                                    {
